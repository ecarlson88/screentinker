@echo off
REM SLI-Signs - Windows Kiosk Setup
...
echo ==================================
echo   SLI-Signs Windows Player
echo ==================================
echo.

REM Create startup shortcut
set STARTUP=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup
set SHORTCUT=%STARTUP%\SLI-Signs.url

echo [InternetShortcut] > "%SHORTCUT%"
echo URL=%PLAYER_URL% >> "%SHORTCUT%"

REM Create a VBS launcher for kiosk mode (Chrome)
set LAUNCHER=%USERPROFILE%\SLI-Signs.vbs
...
copy /Y "%LAUNCHER%" "%STARTUP%\SLI-Signs.vbs" >nul
